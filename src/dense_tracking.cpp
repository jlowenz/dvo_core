...
void DenseTracker::computeLeastSquaresEquationsInverseCompositional(dvo::core::RgbdImage& ref, dvo::core::RgbdImage& cur, const dvo::core::IntrinsicMatrix& intrinsics, const dvo::core::AffineTransform& transformation, dvo::core::LeastSquaresInterface& ls)
{
  ...
  cv::Mat vref;
  ref.intensity.convertTo(vref, CV_8U);  
  cv::Mat vcur;
  cur_warped.intensity.convertTo(vcur, CV_8U);
  // cv::imshow("ref", vref);
  // cv::imshow("cur", vcur);
  ...
  cv::Mat dx;
  ref.intensity_dx.convertTo(dx, CV_8U, 1.0, 127);
  cv::Mat dy;
  ref.intensity_dy.convertTo(dy, CV_8U, 1.0, 127);
  // cv::imshow("dx", dx);
  // cv::imshow("dy", dy);
  ...
  residuals = cur_warped.intensity - ref.intensity;
  cv::Mat vres;
  residuals.convertTo(vres, CV_8U, 1.0, 127);
  // cv::imshow("residuals", vres);
  // cv::waitKey(0);
